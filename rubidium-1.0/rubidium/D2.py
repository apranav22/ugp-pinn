...
from scipy import sqrt, pi, exp, real
...
seterr(
    invalid="ignore"
)  # ignoring invalid warnings as the erf is touchy near ∆=0. No interesting physics there so we can safely ignore it.
...
d21 = sqrt(3) * sqrt(
    3 * epsilon_0 * hbar * Gamma2 * lProbe**3 / (8 * pi**2)
...
def chiRe87(delta, T, Fg, Fe):
    ...
    return K87(T, Fg, Fe) * real(
        D87(2 * pi * (delta + detF87[Fg][Fe]) / (kProbe * u87(T)), T)
    )
...
def chiIm87(delta, T, Fg, Fe):
    ...
    return K87(T, Fg, Fe) * real(
        V87(2 * pi * (delta + detF87[Fg][Fe]) / (kProbe * u87(T)), T)
    )


def chiRe85(delta, T, Fg, Fe):
    """The real part of the susceptibility for 85-Rb"""
    return K85(T, Fg, Fe) * real(
        D85(2 * pi * (delta + detF85[Fg][Fe]) / (kProbe * u85(T)), T)
    )
...
def chiIm85(delta, T, Fg, Fe):
    ...
    return K85(T, Fg, Fe) * real(
        V85(2 * pi * (delta + detF85[Fg][Fe]) / (kProbe * u85(T)), T)
    )
